 ...
 *   - Subsequent mesh rebuilds → preserve user's orbit/pan/zoom
 *   - Explicit frameCurrentModel → always frame (user-initiated)
 *
 * The scene.ts VoxelForgeScene class uses the same `initialCameraFramed` flag
 * pattern tested here. Since the class requires Three.js/WebGL (unavailable
 * in Node test environment), we test the decision logic in isolation.
 */
 ...
 * Mirrors the logic in VoxelForgeScene.buildMeshFromSnapshot.
 */
function shouldFrameCamera(
...
describe("camera framing decision logic", () => {
