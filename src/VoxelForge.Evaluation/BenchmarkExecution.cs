...
public sealed class BenchmarkRunner
{
    ...
    private readonly IBenchmarkExecutionBackend _stdioBackend;
    ...
    public BenchmarkRunner()
        : this(
            ...
            new StdioCommandBenchmarkBackend(new InProcessStdioCommandTransportFactory()),
    ...
    public BenchmarkRunner(
        IBenchmarkExecutionBackend mcpBackend,
        IBenchmarkExecutionBackend stdioBackend,
        BenchmarkArtifactWriter artifactWriter,
        BenchmarkComparisonService comparisonService)
    {
        _mcpBackend = mcpBackend;
        _stdioBackend = stdioBackend;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        IBenchmarkExecutionBackend backend = ResolveBackend(executionPlan.Backend);
        ...
        for (int i = 0; i < executionPlan.Runs.Count; i++)
        {
            ...
            BenchmarkRunExecutionResult execution = await backend.RunAsync(new BenchmarkRunExecutionRequest
            {
                PlannedRun = run,
                InputRoot = inputRoot,
            ...
            _artifactWriter.WriteRunArtifacts(suite, new BenchmarkRunArtifactRequest
            {
                ...
                StdioTranscriptJsonl = execution.StdioTranscriptJsonl,
    ...
    private IBenchmarkExecutionBackend ResolveBackend(string backend)
    {
        if (string.Equals(backend, BenchmarkPlanner.DefaultBackend, StringComparison.Ordinal))
            return _mcpBackend;

        if (string.Equals(backend, "stdio", StringComparison.Ordinal))
            return _stdioBackend;

        throw new InvalidOperationException($"Execution backend '{backend}' is not implemented.");
    }
